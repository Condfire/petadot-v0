...
WHERE table_name = 'pets' 
AND table_schema = 'public'
AND column_name IN ('status', 'rejection_reason', 'name', 'description')
ORDER BY ordinal_position;

-- Verificar se existem pets com status rejected
SELECT 
    COUNT(*) as total_pets,
    COUNT(CASE WHEN status = 'rejected' THEN 1 END) as rejected_pets,
    COUNT(CASE WHEN rejection_reason IS NOT NULL THEN 1 END) as pets_with_rejection_reason
FROM pets;
