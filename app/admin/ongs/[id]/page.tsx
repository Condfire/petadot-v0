...
export default async function OngDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!session) {
    redirect("/login?callbackUrl=/admin/ongs/" + id)
  }
        ...
        <div>
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button variant="outline" className="w-full" asChild>
                <Link href={`/admin/ongs/${ong.id}/edit`}>Editar ONG</Link>
              </Button>

              <Button variant="destructive" className="w-full" asChild>
                <Link href={`/admin/ongs/${ong.id}/delete`}>Excluir ONG</Link>
              </Button>
            </CardContent>
          </Card>
