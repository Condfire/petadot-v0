...
export default function CadastrarPetAdocaoClient() {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    console.log("📤 Submitting adoption pet form...")

    startTransition(async () => {
      try {
        const result = await createAdoptionPet(formData)

        if (result.success) {
          console.log("✅ Pet cadastrado com sucesso:", result.data)
          toast.success("Pet cadastrado para adoção com sucesso!")
          ...
          console.error("❌ Erro ao cadastrar pet:", result.error)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center">Cadastrar Pet para Adoção</CardTitle>
        </CardHeader>
        <CardContent>
          <form action={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <Textarea
                id="description"
                name="description"
                placeholder="Descreva o pet, personalidade, cuidados especiais..."
                rows={4}
                disabled={isPending}
              />
            </div>
