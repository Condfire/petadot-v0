...
export default function DeleteLostPetPage({ params }: { params: { id: string } }) {
  return (
    <RequireAuth>
      <DeletePetContent id={params.id} type="lost" />
    </RequireAuth>
  )
}
      ...
      {isLoading ? (
        ...
        <Card className="max-w-md mx-auto">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Você está prestes a excluir permanentemente o pet perdido. Esta ação não pode ser desfeita.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col items-center">
            ...
            <p className="text-muted-foreground mt-1">Perdido em {pet.last_seen_location || "Local não informado"}</p>
          </CardContent>
          <CardFooter className="flex justify-between">
