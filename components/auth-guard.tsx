...
export default function AuthGuard({ children, fallback, redirectTo = "/login" }: AuthGuardProps) {
      ...
      try {
        ...
        timeoutId = setTimeout(() => {
          if (isMounted && isChecking) {
            ...
            router.replace(`${redirectTo}?redirectTo=${encodeURIComponent(window.location.pathname)}&error=timeout`)
          }
        ...
        if (sessionError) {
          ...
          if (isMounted) {
            ...
            router.replace(`${redirectTo}?redirectTo=${encodeURIComponent(window.location.pathname)}&error=session`)
          }
          return
        }
        ...
        if (isMounted) {
          ...
          router.replace(`${redirectTo}?redirectTo=${encodeURIComponent(window.location.pathname)}&error=unknown`)
        }
      }
    }
  ...
  }, [router, supabase, redirectTo, isChecking])
    ...
    return (
      fallback || (
        <div className="flex flex-col items-center justify-center min-h-[50vh]">
          ...
          <button onClick={() => router.push(redirectTo)} className="px-4 py-2 bg-primary text-white rounded-md">
            Ir para login
          </button>
        </div>
      )
